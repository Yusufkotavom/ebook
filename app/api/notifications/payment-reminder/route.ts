  ...
  try {
    ...
    if (order.status !== 'pending') {
      return NextResponse.json({ 
        success: false, 
        message: "Order is no longer pending, reminder not needed" 
      })
    }
    ...
    const emailData = emailService.generatePaymentReminderEmail(reminderEmailData)
    ...
    const { error: logError } = await supabase.from("notifications").insert([
      {
        ...
        recipient: email,
        message: `Payment reminder sent for order #${orderId.slice(0, 8)}`,
